...
class MySQLConcentratorPacketTest extends MySQLConcentratorBaseTest
{
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_binary("\x32abc");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_binary("\xfc\x33\x25");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_binary("\xfd\x33\x25\xfd");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_binary("\xfe\x33\x25\xfd\x03\xd1\xa1\x52\x22");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_string("\x0512345");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_string("\xfc\x05\x0012345");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_length_coded_binary("\xfbabc");
  ...
  {
    list($result, $length) = MySQLConcentrator\Packet::unmarshall_null_terminated_string("abc\x00def");
  ...
  function testParse()
  {
    $packet = new MySQLConcentrator\Packet("\x07\x00\x00\x02\x00\x00\x00\x02\x00\x00\x00");
    $packet->parse('result', 'result_set');
    $this->assertEqual(7, $packet->length);
    $this->assertEqual(2, $packet->number);
    $this->assertEqual(MySQLConcentrator\Packet::RESPONSE_OK, $packet->type);
  ...
  function testQuery()
  {
    $packet = new MySQLConcentrator\Packet("\x28\x00\x00\x00\x03\x53\x45\x4c\x45\x43\x54\x20\x2a\x20\x46\x52\x4f\x4d\x20\x66\x6f\x6f\x20\x57\x48\x45\x52\x45\x20\x76\x61\x6c\x75\x65\x20\x3d\x20\x27\x66\x69\x72\x73\x74\x27");
    $packet->parse('command');
    $this->assertEqual(MySQLConcentrator\Packet::COM_QUERY, $packet->type);
    $this->assertEqual("SELECT * FROM foo WHERE value = 'first'", $packet->attributes['statement']);
  }

  function testResultSetHeaderPacket()
  {
    $packet = new MySQLConcentrator\Packet("\x01\x00\x00\x01\x02");
    $packet->parse('result', 'result_set');
    $this->assertEqual(MySQLConcentrator\Packet::RESPONSE_RESULT_SET, $packet->type);
    $this->assertEqual(2, $packet->attributes['field_count']);
  }

  function testResultSetFieldPacket()
  {
    $packet = new MySQLConcentrator\Packet("\x37\x00\x00\x02\x03\x64\x65\x66\x17\x6d\x79\x73\x71\x6c\x5f\x63\x6f\x6e\x63\x65\x6e\x74\x72\x61\x74\x6f\x72\x5f\x74\x65\x73\x74\x03\x66\x6f\x6f\x03\x66\x6f\x6f\x02\x69\x64\x02\x69\x64\x0c\x3f\x00\x0b\x00\x00\x00\x03\x03\x42\x00\x00\x00");
    $packet->parse('result', 'field');
    $this->assertEqual(MySQLConcentrator\Packet::RESPONSE_FIELD, $packet->type);
  ...
  {
    $packet = new MySQLConcentrator\Packet("\x05\x00\x00\x04\xfe\x00\x00\x22\x00");
    $packet->parse('result', 'result_set');
    $this->assertEqual(0, $packet->attributes['warning_count']);
  ...
  {
    $packet = new MySQLConcentrator\Packet("\x08\x00\x00\x05\x01\x31\x05\x66\x69\x72\x73\x74");
    $packet->parse('result', 'row_data', 2);
    $this->assertEqual(MySQLConcentrator\Packet::RESPONSE_ROW_DATA, $packet->type);
    $this->assertEqual(array(0 => 1, 1 => 'first'), $packet->attributes['column_data']);
  }

  function testQuitPacket()
  {
    $packet = new MySQLConcentrator\Packet("\x01\x00\x00\x00\x01");
    $packet->parse('command');
    $this->assertEqual(MySQLConcentrator\Packet::COM_QUIT, $packet->type);
  }

  function testFieldListPacket()
  {
    $packet = new MySQLConcentrator\Packet("\x09\x00\x00\x00\x04\x61\x63\x74\x69\x6f\x6e\x73\x00");
    $packet->parse('command');
    $this->assertEqual(MySQLConcentrator\Packet::COM_FIELD_LIST, $packet->type);
    $this->assertEqual('actions', $packet->attributes['table_name']);
  }

  function testMarshallLittleEndian()
  {
    $result = MySQLConcentrator\Packet::marshall_little_endian_integer(22, 1);
